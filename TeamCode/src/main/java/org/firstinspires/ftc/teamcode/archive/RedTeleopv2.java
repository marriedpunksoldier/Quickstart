...
public class RedTeleopv2 extends OpMode {
    ...
    private void startAutoMove() {
        ...
        currentTargetRPM = AUTO_MOVE_SHOOTER_POWER * ShooterConfig.MOTOR_FREE_SPEED_RPM;
        shooterRunning = true;
    ...
    private void startShooter() {
        ...
        currentTargetRPM = powerLevel * ShooterConfig.MOTOR_FREE_SPEED_RPM;
        shooterRunning = true;
        shooterVelocityReached = false;
    }
    ...
    private void updateShooterVelocityForAuto() {
        if (limelightHasTarget && autoPowerLevel > 0) {
            double newTargetRPM = autoPowerLevel * ShooterConfig.MOTOR_FREE_SPEED_RPM;
            if (Math.abs(newTargetRPM - currentTargetRPM) / currentTargetRPM > 0.02) {
                currentTargetRPM = newTargetRPM;
            }
        }
    }

    private void updateShooterOutput() {
        if (!shooterRunning || currentTargetRPM <= 0) { shooter.setPower(0); return; }
